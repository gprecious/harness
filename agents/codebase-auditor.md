...
- Unused imports: `Grep` for `^import\s+.*from\s+['"].*['"]` (or `^from\s+\S+\s+import` in Python), then for each imported symbol, `Grep` the rest of the file for any use (use a word-boundary regex like `\bsymbol\b` against file contents, excluding the import line itself). Flag imports with zero non-import-line matches. For namespace/default imports (e.g., `import * as fs from "fs"`, `import _ from "lodash"`), check for any occurrence of the binding name outside import lines.
- Unused local variables / parameters: `Grep` for local declarations (`\b(const|let|var)\s+(\w+)` in TS/JS; `^\s*(\w+)\s*=` inside `def` bodies in Python), then `Grep` the rest of the same function/scope for additional occurrences of the declared name. Flag declarations with zero additional matches. For TypeScript, if `tsconfig.json` sets `noUnusedLocals: true` and `tsc --noEmit` is available, prefer the compiler's diagnostics; otherwise fall back to the Grep recipe above.
- Commented-out code blocks: `Grep -n` for lines beginning with `//`, `#`, or `/*`, then detect contiguous runs >= 3 lines containing code-like tokens (`(`, `;`, `=`, `return`, `function`, `def`, `class`). A block is "contiguous" if each line is a comment and no non-comment/non-blank line separates them.
